...
if ({foreach from=$field.options item=option}(!document.getElementById('form_{$field.field}_{$option|replace:' ':'_'|replace:'$':''}').checked){if !$smarty.foreach.default.last} && {/if}{/foreach})
{literal}{{/literal}errors[i++]='{$fields[f].display} is a required field';{literal}}{/literal}
{else}
