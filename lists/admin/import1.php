 ...
 */
  ...
  if (isset($_REQUEST['import_field_delimiter'])) {
    $import_field_delimiter = $_REQUEST['import_field_delimiter'];
    if ($import_field_delimiter == "TAB") $import_field_delimiter = "\t";
  } else {
    ...
    while (list($email,$data) = each ($user_list)) {
      ...
      if(strlen($email) > 4) {
        ...
        if (Sql_affected_rows()) {
          ...
          $uniqid = getUniqid();
//can't input directly to the phplist user table.
          //$query = sprintf('INSERT INTO %s (email,entered,confirmed,uniqid,htmlemail) values("%s",now(),%d,"%s","%s")', $tables["user"],$email,$notify != "yes",$uniqid,$htmlemail);
          $query = sprintf('INSERT INTO %s (us_email,entered,confirmed,htmlemail) values("%s",now(),%d,"%s")', 'user',$email,$notify != "yes",$htmlemail);
