  ...
  foreach ($email_list as $line) {
    ...
    if ($_SESSION["test_import"]) {
      ...
      if (!empty($user["systemvalues"]["foreign key"])) {
        ...
        if ($clashcheck[0] != $existing_user["id"]) {
          ...
          if (!$_SESSION["retainold"]) {
            Sql_Query(sprintf('update %s set us_email = "%s" where us_email = "%s"', 'user', "duplicate$c " .
      ...
      if ($exists) {
        ...
        $query = sprintf('INSERT INTO %s (us_email,entered,confirmed)
                  values("%s",now(),%d)', 'user', $user["systemvalues"]["email"], $confirmed);
        ...
        if (!$userid) {
          ...
          while (!$userid) {
            $c++;
            $query = sprintf('INSERT INTO %s (us_email,entered,confirmed)
                          values("%s",now(),%d)', 'user', $user["systemvalues"]["email"] .
            " ($c)", 0, $uniqid);
            $result = Sql_query($query, 1);
            $userid = Sql_insert_id();
          }
      ...
      if ($new || (!$new && $_SESSION["overwrite"] == "yes")) {
        ...
        if ($query) {
          ...
          $query=str_replace("email","us_email",$query);
          Sql_Query("update ignore user set $query where userid = $userid");
        }
